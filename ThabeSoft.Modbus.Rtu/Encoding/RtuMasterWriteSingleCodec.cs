...
public sealed class RtuMasterWriteSingleCodec : IMasterWriteSingleCodec
{
    ...
    public static Result<RtuWriteSingleCoilResponseHeader> DecodeCoilResponse(ReadOnlySpan<byte> source, in RtuWriteSingleLayout layout)
    {
        ...
        if (!function_code_result) return function_code_result.PropagateError<RtuWriteSingleCoilResponseHeader>();

        // Crc
        var crc_result = source[layout.CrcRange]
            .ToWord(Endianness.LittleEndian);
        if (!crc_result) return crc_result.PropagateError<RtuWriteSingleCoilResponseHeader>();

        // 验证
        var validate_result = Crc16.Validate(source[layout.PayloadRange], crc_result.Value);
        if (!validate_result) return validate_result.PropagateError<RtuWriteSingleCoilResponseHeader>();
        ...
        if (!address_result) return address_result.PropagateError<RtuWriteSingleCoilResponseHeader>();
        // 数据
        var value_result = source[layout.DataRange].ToWord()
            .Bind(ModbusHelper.ToModbusCoilValue);
        if (!value_result) return value_result.PropagateError<RtuWriteSingleCoilResponseHeader>();

        // 拷贝数据
        return new RtuWriteSingleCoilResponseHeader(
            slaveId: slave_id,
    ...
    public static Result<RtuWriteSingleRegisterResponseHeader> DecodeRegisterResponse(ReadOnlySpan<byte> source, in RtuWriteSingleLayout layout)
    {
        ...
        var function_code_result = FunctionCode
            .FromCode(source[layout.FunctionCodeIndex])
            .Where(x => FunctionCode.WriteSingleCoil == x);
        if (!function_code_result) return function_code_result.PropagateError<RtuWriteSingleRegisterResponseHeader>();

        // Crc
        var crc_result = source[layout.CrcRange]
            .ToWord(Endianness.LittleEndian);
        if (!crc_result) return crc_result.PropagateError<RtuWriteSingleRegisterResponseHeader>();

        // 验证
        var validate_result = Crc16.Validate(source[layout.PayloadRange], crc_result.Value);
        if (!validate_result) return validate_result.PropagateError<RtuWriteSingleRegisterResponseHeader>();
        ...
        var address_result = source[layout.AddressRange]
            .ToWord(Endianness.BigEndian);
        if (!address_result) return address_result.PropagateError<RtuWriteSingleRegisterResponseHeader>();
        // 数据
        var value_result = source[layout.DataRange]
            .ToWord(Endianness.BigEndian);
        if (!value_result) return value_result.PropagateError<RtuWriteSingleRegisterResponseHeader>();

        // 拷贝数据
        return new RtuWriteSingleRegisterResponseHeader(
            slaveId: slave_id,
            address: address_result.Value,
            value: value_result.Value,
            crc: crc_result.Value);
