...
public sealed class RtuReadCodec : IReadCodec
{
    ...
    Result<ReadResponseHeader> IReadCodec.DecodeCoilsResponse(ReadOnlySpan<byte> source, Span<bool> destination)
    {
        return DecodeCoilsResponse(source, destination).Map(x => (ReadResponseHeader)x);
    }
    Result<ReadResponseHeader> IReadCodec.DecodeRegistersResponse(ReadOnlySpan<byte> source, Span<ushort> destination)
    {
