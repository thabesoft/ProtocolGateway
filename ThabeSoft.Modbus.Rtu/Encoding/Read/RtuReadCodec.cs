...
/// Rtu读请编码器
/// </summary>
public sealed class RtuReadCodec : IReadCodec
{
    private RtuReadCodec() { }
    public static RtuReadCodec Instance { get; } = new();


    public Result<int> EncodeRequest(Span<byte> destination, in ReadRequestHeader header)
    {
        var layout = RtuReadRequestLayout.Instance;
        return EncodeRequest(destination, header, layout).Then(layout.TotalLength);
    }
    public Result<ReadResponseHeader> DecodeCoilsResponse(ReadOnlySpan<byte> source, Span<bool> values)
    {
        var data_length = ModbusHelper.GetColisToByteLength(values.Length);
        var layout = RtuReadResponseLayout.FromDataLength(data_length);

        return DecodeCoilsResponse(source, values, layout).Map(x => (ReadResponseHeader)x);
    }
    public Result<ReadResponseHeader> DecodeRegistersResponse(ReadOnlySpan<byte> source, Span<ushort> values)
    {
        var data_length = ModbusHelper.GetRegistersToByteLength(values.Length);
        var layout = RtuReadResponseLayout.FromDataLength(data_length);

        return DecodeRegistersResponse(source, values, layout).Map(x => (ReadResponseHeader)x);
    }
    ...
    public static Result<RtuReadResponseHeader> DecodeResponse(ReadOnlySpan<byte> source, in RtuReadResponseLayout layout)
    {
        // 数据不足
