...
/// 写多值编码器
/// </summary>
public sealed class RtuMasterWriteMultipleCodec : IMasterWriteMultipleCodec
{
    private RtuMasterWriteMultipleCodec() { }
    public static RtuMasterWriteMultipleCodec Instance { get; } = new();
    ...
    {
        return DecodeCoilsResponse(source).Then(x => (WriteMultipleResponseHeader)x);
    }
    Result<WriteMultipleResponseHeader> IMasterWriteMultipleCodec.DecodeRegistersResponse(ReadOnlySpan<byte> source)
    {
        return DecodeRegistersResponse(source).Then(x => (WriteMultipleResponseHeader)x);
    }
    ...
    public static Result<int> EncodeCoilsRequest(Span<byte> destination, in WriteMultipleRequestHeader header, ReadOnlySpan<bool> values)
    {
        var layout_result = WriteCoilsQuantity.Create(values.Length)
            .Then(RtuWriteMultipleRequestLayout.FromQuantity);
    ...
    public static Result EncodeCoilsRequest(Span<byte> destination, in WriteMultipleRequestHeader header, ReadOnlySpan<bool> values, in RtuWriteMultipleRequestLayout layout)
    {
        ...
        if (destination.Length < layout.TotalLength)
            return BufferTooSmall("写多个线圈", layout.TotalLength, destination.Length);
