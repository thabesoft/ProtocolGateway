...
public sealed class RtuMasterReadCodec : IMasterReadCodec
{
    ...
    {
        return DecodeCoilsResponse(source, destination).Then(x => (ReadResponseHeader)x);
    }
    Result<ReadResponseHeader> IMasterReadCodec.DecodeRegistersResponse(ReadOnlySpan<byte> source, Span<ushort> destination)
    {
        return DecodeRegistersResponse(source, destination).Then(x => (ReadResponseHeader)x);
    }
    ...
    {
        var layout_result = ReadCoilsQuantity.Create(destination.Length).Then(RtuReadResponseLayout.FromQuantity);
    ...
    {
        var layout_result = ReadRegistersQuantity.Create(destination.Length).Then(RtuReadResponseLayout.FromQuantity);
        if (!layout_result.IsSuccess) return layout_result.Cast<RtuReadResponseHeader>();
