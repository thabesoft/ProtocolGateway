...
public readonly struct RtuReadResponseHeader
{
    ...
    {
        if (!functionCode.IsReadCoils) return Result.Error<RtuReadResponseHeader>("无效功能吗");

        var quantity_result = ReadCoilsQuantity.Create(quantity);
        if (!quantity_result.IsSuccess) return quantity_result.Cast<RtuReadResponseHeader>();
    ...
    {
        if (!functionCode.IsReadRegisters) return Result.Error<RtuReadResponseHeader>("无效功能吗");
    ...
    public static Result<RtuReadResponseHeader> Create(byte slaveId, FunctionCode functionCode, int quantity, ushort crc)
    {
        ...
        return Result.Error<RtuReadResponseHeader>($"读请求头创建失败, 不支持的功能码: {functionCode}");
    }
