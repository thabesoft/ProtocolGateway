    ...
    IBinarySerializer<double>
{
    ...
    Result<long> IBinarySerializer<long>.Deserialize(ReadOnlySpan<byte> source) => From(source).Then(x => (long)x);
    Result<double> IBinarySerializer<double>.Deserialize(ReadOnlySpan<byte> source) => From(source).Then(x => (double)x);
